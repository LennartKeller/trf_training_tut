...
if __name__ == "__main__":
    ...
    model_config = BertForMultiHeadConfig.from_pretrained(
        "bert-base-german-cased", num_labels=num_classes, num_multi_labels=num_labels
    )
    model = BertForMultiHeadModel.from_pretrained(
        "bert-base-german-cased", config=model_config
    )

    tokenization = make_tokenization_func(
        tokenizer=tokenizer, text_column="text", padding=True, truncation=True
    )
