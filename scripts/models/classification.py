...
class FeedForwardClassifier(nn.Module):
    ...
    def __init__(self, input_features, n_classes, hidden_layer_sizes=None):
        ...
        output_layer = nn.Linear(in_features=hidden_layer_sizes[-1], out_features=n_classes)
        layers = [input_layer]
        for idx, hidden_size in enumerate(hidden_layer_sizes):
            in_features = hidden_layer_sizes[idx - 1] if idx > 0 else input_layer.out_features
